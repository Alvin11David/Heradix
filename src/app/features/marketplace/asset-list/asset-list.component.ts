    ...
    <section class="amx-cal-section">
      <div class="amx-cal-section__header">
        ...
        <div class="amx-cal-section__nav">
          <button class="amx-cal-section__view-all" type="button" (click)="resetCalendar()">
            View all events
          ...
          <span class="amx-cal-section__page">{{ calPage() + 1 }} / {{ calEvents.length }}</span>
      ...
      <div class="amx-cal-cards">
        <div *ngFor="let ev of visibleCalEvents(); let i = index"
          ...
          <div class="amx-cal-card__month">{{ ev.month }}</div>
          ...
          <div class="amx-cal-card__footer">
            <div class="amx-cal-card__footer-left">
              <span class="amx-cal-card__icon-wrap">{{ ev.emoji }}</span>
              <div>
            ...
            <button class="amx-cal-card__browse" type="button"
                    (click)="$event.stopPropagation(); selectEvent(ev)">
              <span>Browse {{ ev.assets.length }} resources</span>
...
export class AssetListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  selectEvent(ev: CalendarEvent): void {
    ...
    requestAnimationFrame(() => {
      const el = this.el.nativeElement.querySelector('.amx-cal-resources');
  ...
  resetCalendar(): void {
    ...
    this.activeEvent.set(this.calEvents[0]);
    requestAnimationFrame(() => {
      const el = this.el.nativeElement.querySelector('.amx-cal-section');
      if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
  }
